...
- La racine du projet inclut un fichier `.npmrc` minimal (registre public + `strict-ssl=false`). Laissez vos variables d’environnement `HTTP(S)_PROXY` actives si votre réseau en a besoin, ou configurez `npm_config_http_proxy` / `npm_config_https_proxy` en conséquence.
- Assurez-vous que votre réseau autorise l’accès à `https://registry.npmjs.org/` ou au registre interne que vous utilisez.
